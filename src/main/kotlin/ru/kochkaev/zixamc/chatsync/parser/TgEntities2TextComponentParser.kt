...
object TgEntities2TextComponentParser {
    ...
    fun parse(message: TgMessage, text: String, entities: List<TgEntity>?) =
        parse(TextParser.resolveMessageLink(message), text, entities)
            ...
            if (isLegacy || i == text.length-1) {
                ...
                currentEntities.forEach {
                    when (it.type) {
                        ...
                        TgEntityType.TEXT_LINK -> tempComponent = ChatSyncBotCore.lang.minecraft.link.getTextComponent(
                            plainPlaceholders = listOf("url" to it.url!!),
                            componentPlaceholders = listOf("title" to tempComponent.build()),
                        ).toBuilder()
                        TgEntityType.URL -> tempComponent = ChatSyncBotCore.lang.minecraft.link.getTextComponent(
                            plainPlaceholders = listOf("url" to tempText),
                            componentPlaceholders = listOf("title" to tempComponent.build()),
                        ).toBuilder()
                        TgEntityType.MENTION -> tempComponent = ChatSyncBotCore.lang.minecraft.mention.getTextComponent(
                            plainPlaceholders = listOf("mention" to tempText),
                            componentPlaceholders = listOf("title" to tempComponent.build())
                        ).toBuilder()
                        TgEntityType.HASHTAG, TgEntityType.CASHTAG -> tempComponent = ChatSyncBotCore.lang.minecraft.hashtag.getTextComponent(
                            plainPlaceholders = listOf("url" to (messageLink?:"")),
                            componentPlaceholders = listOf("title" to tempComponent.build())
                        ).toBuilder()
                        TgEntityType.SPOILER -> isSpoiler = true
                        TgEntityType.CODE, TgEntityType.PRE -> tempComponent = ChatSyncBotCore.lang.minecraft.code.getTextComponent(
...
//                }
    ...
    class SpoilerComponent {
        ...
        fun build() = ChatSyncBotCore.lang.minecraft.spoiler.getTextComponent(
            plainPlaceholders = listOf("placeholder" to ChatSyncBotCore.lang.minecraft.spoilerReplaceWithChar?.repeat(rawList.joinToString("").length).toString()),
