    ...
    val processor: ProcessFormat?
        get() = ProcessFormat {
            ...
            for (result in regex.findAll(content)) {
                ...
                } else (if (!success) parsed else it.subNode).add(Node.of(result.value, FormatTypes.REGULAR))
                i += len
            }
    ...
    companion object {
        ...
        fun isAllInOneRegular(list: ArrayList<Node>, start: Int, end: Int) : Boolean {
            ...
            for (node in list) {
                ...
                if (node.format == FormatTypes.REGULAR && (start in i..len && end in i..len)) return true
                i+=len
            }
            return false
        }
        fun isAllInRegular(list: ArrayList<Node>, start: Int, end: Int) : Boolean {
            ...
            for (node in list) {
                val content = node.build()
                val len = content.length
                if (node.format != FormatTypes.REGULAR && (start in i..<len || end in i..len)) isAllInRegular = false
                i+=len
            }
